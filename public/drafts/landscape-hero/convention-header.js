...
async function boot() {
  ...
  scene.add(new THREE.HemisphereLight(0xfffaeb, 0x79885d, 1.25));
  const sun = new THREE.DirectionalLight(0xffeed1, 2.4);
  ...
  const dirt = new THREE.InstancedMesh(
    ...
    new THREE.MeshToonMaterial({ color: 0xa16c3c, gradientMap: ramp }),
    dirtPool.length,
  );
  ...
  const holes = new THREE.InstancedMesh(
    holeGeometry,
    new THREE.MeshBasicMaterial({ color: 0x47321e, side: THREE.DoubleSide }),
    count,
  );
  holes.count = 0;
