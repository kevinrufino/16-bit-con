...
async function boot() {
  ...
  // Recolor blue/purple clothing into the leaf family; blue belongs to skies.
  for (const source of sources) source.scene.traverse((mesh) => {
    if (!mesh.isMesh) return;
    const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    for (const material of materials) {
      if (!material.color) continue;
      const hsl = material.color.getHSL({ h: 0, s: 0, l: 0 });
      if (hsl.h > 0.45 && hsl.h < 0.85 && hsl.s > 0.2)
        material.color.set(palette["color-heading"]);
    }
  });
